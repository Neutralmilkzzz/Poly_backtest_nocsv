 ...
 *** FIXED: 用 up/down midpoint 判结算, 不再用 btc_diff ***
═══════════════════════════════════════════════════════════════
...
结算判定: 用 285-298s 窗口内最后的 up_midpoint 判断
  up_midpoint > 0.5 → UP 赢
  up_midpoint < 0.5 → DOWN 赢
  无数据或 up_midpoint == 0.5 → 跳过
"""
...
OUT_DIR  = r'C:\Users\ZHAOKAI\Poly_backtest_Final\results\three_strategies_fixed'
...
# ─────────────────────────────────────────────────────
# SETTLEMENT: 用 up/down 价格判定，完全不用 btc_diff
# ─────────────────────────────────────────────────────
def determine_settlement(df):
    """用 285-298s 窗口内最后一个 up_midpoint 判结算。
    如果没有，往前找最后一个有效 up_midpoint（elapsed >= 240）。
    返回 'up' / 'down' / None"""
    # 优先：285-298s
...
def load_round(fpath):
    ...
    try:
        ...
        df.attrs['settlement'] = settlement
...
def compute_features(df):
    ...
    if len(tail) > 5:
        ...
        if len(last30) > 0:
            ...
            feat['tail_spread'] = us.mean() if len(us) > 0 else np.nan
        else:
            feat['tail_spread'] = np.nan

        # Big moves count
        up_changes = up_mid.diff().abs()
        feat['n_big_moves'] = int((up_changes > 0.05).sum())
        feat['max_single_move'] = float(up_changes.max()) if len(up_changes) > 1 else 0
    else:
        feat['tail_up_range'] = 0
        feat['tail_dn_range'] = 0
        feat['tail_range'] = 0
        feat['up_mid_240'] = np.nan
        feat['up_mid_250'] = np.nan
        feat['direction_240'] = None
...
# ─────────────────────────────────────────────────────
def strategy_grid(df, entry_start=0, entry_end=90, sell_start=0, sell_end=180,
                  buy_threshold=0.25, profit_target=0.01, shares=10):
    """Grid: buy UP at ask <= 0.25, sell at bid >= 0.26 within sell window.
    If not sold by 180s, market sell at last bid. If no bids, hold to settlement."""
    ...
    for side in ['up', 'down']:
        ...
        cheap = entry_df[(entry_df[ask_col].fillna(999) > 0) &
                         (entry_df[ask_col].fillna(999) <= buy_threshold)]
        if len(cheap) == 0:
            continue

        entry_row = cheap.iloc[0]
        entry_price = float(entry_row[ask_col])
        entry_time = entry_row['elapsed']
        profit_price = entry_price + profit_target

        sell_df = df[(df['elapsed'] > entry_time) & (df['elapsed'] <= sell_end)]
...
# ─────────────────────────────────────────────────────
def strategy_tail_momentum(df, check_time=250, threshold=0.55, shares=10):
...
print(f'Settlement method: up_midpoint at 285-298s (NO btc_diff)')
print(f'{"="*60}\n')
...
for i, fpath in enumerate(files):
    ...
    if (i + 1) % 500 == 0:
        print(f'  {i+1}/{len(files)} loaded={loaded} skipped={skipped} ({time.time()-t0:.0f}s)')

elapsed = time.time() - t0
print(f'\nDone in {elapsed:.0f}s, {loaded} rounds loaded, {skipped} skipped')

df_all = pd.DataFrame(all_results)
df_all.to_csv(os.path.join(OUT_DIR, 'three_strategies_fixed.csv'), index=False)

# ═══════════════════════════════════════════════════════
# SETTLEMENT SANITY CHECK
# ═══════════════════════════════════════════════════════
up_count = (df_all['f_settlement'] == 'up').sum()
dn_count = (df_all['f_settlement'] == 'down').sum()
print(f'\nSettlement distribution: UP={up_count} ({up_count/len(df_all)*100:.1f}%), '
      f'DOWN={dn_count} ({dn_count/len(df_all)*100:.1f}%)')
...
def analyze_strategy(df_all, prefix, name):
    ...
    # Side breakdown
    for side in ['up', 'down']:
        sub = traded[traded[f'{prefix}_side'] == side]
        if len(sub) > 0:
            wr = (sub[f'{prefix}_pnl'] > 0).mean() * 100
            tp = sub[f'{prefix}_pnl'].sum()
            print(f'  Buy {side.upper()}: {len(sub)} trades, WR={wr:.1f}%, PnL={tp:+.1f}')
    ...
    for fcol in feature_cols:
        ...
        try:
            ...
            if len(grouped) >= 2:
                ...
                delta = hi - lo
                if abs(delta) > 0.05:
                    print(f'\n  {fcol}: (Q1 loss={lo:.1%}, Q4 loss={hi:.1%}, delta={delta:+.1%})')
...
# Run analysis
print(f'\n{"#"*60}')
print(f'#  THREE-STRATEGY ANALYSIS (FIXED SETTLEMENT)')
print(f'{"#"*60}')

tA = analyze_strategy(df_all, 'A', 'Classic Grid -- Buy 0.25, Sell 0.26, 0-90s')
tB = analyze_strategy(df_all, 'B', 'Tail Momentum -- Follow direction at t=250')
tC = analyze_strategy(df_all, 'C', 'Tail Dip Buy -- Buy ask<0.20 in tail')
...
print(f'{"="*60}')
print(f'\n{"Strategy":<25} {"Trades":>7} {"WinRate":>8} {"PnL":>9} {"BestSignal":<25}')
print('-' * 80)
